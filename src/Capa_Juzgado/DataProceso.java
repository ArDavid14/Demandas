...
public class DataProceso {
    ...
    public ArrayList<DataProceso> ListarProceso() {
        ...
        try {
            ...
            while (tabla.next()) {
                ...
                objproceso.setPr_numradicado(tabla.getLong("Numero_Radicado"));
    ...
    public String GuardarDetallep() {
        Conexion objmod = new Conexion();
        String cad = "insert into PROCESO values('"+this.getPr_numradicado()+"','"
                + this.getPr_fechain() + "','" + this.getPr_fkcontr() + "')";
        return objmod.Ejecutar(cad);
    }

    public long getPr_numradicado() {
        return pr_numradicado;
    }

    public void setPr_numradicado(long pr_numradicado) {
